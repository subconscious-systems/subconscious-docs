...
2. **Confirm prerequisites**: make sure the API Gateway is deployed, OrangeLine workers are reachable, required credentials are available, and first admins and pilot users are identified.
3. **Configure credentials and routes**: add only the credentials needed for routes, then create the initial model route to OrangeLine.
...
- Pair gateway metrics with OrangeLine metrics such as GPU utilization, queue depth, and token throughput.
...
- Initial OrangeLine routes are active and healthy.
