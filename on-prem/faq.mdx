...
Closed hosted tools can send prompts, code, and engineering context outside your trust boundary. The ORANGE Line gives your team frontier-level coding-agent intelligence while keeping API Gateway traffic, Inference Runtime execution, and deployment policy inside infrastructure you control.
...
No. The customer-hosted deployment is designed so the API Gateway and Inference Runtime run in your cloud account or controlled environment. You retain control over deployment, access, monitoring, networking, data, and change management.

See [Compliance](/on-prem/trust-center/compliance).

## Where does The ORANGE Line run?
...
See [API Gateway setup](/on-prem/deployments/gateway-setup).
