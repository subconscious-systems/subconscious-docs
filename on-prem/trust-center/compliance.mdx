---
title: "Compliance"
description: "Security, SOC 2 review posture, data handling, support access, and supply-chain assurance"
icon: "shield-check"
---

The customer-hosted Subconscious Inference System is designed for enterprises that want high-performance coding-agent inference while retaining control over cloud environment, data, networking, runtime operations, and software deployment cadence.

This page explains how the deployment model supports security review, SOC 2 vendor assessment, and long-term operational trust. It is written for security, compliance, platform, and engineering teams evaluating Subconscious.

## Short answer

The customer-hosted Subconscious Inference System is designed so customers can operate it without compromising their SOC 2 audit posture.

For production customer-hosted deployments:

- The runtime plane runs in the customer's cloud account or customer-controlled environment.
- Customer requests, prompts, completions, logs, keys, and runtime data remain in the customer environment unless the customer explicitly chooses otherwise.
...
- Subconscious does not need ingress into the customer's cloud account by default.
- Subconscious does not control production availability.
- The customer controls versioning, deployment cadence, networking, access, monitoring, and runtime operations.
...
This model lets the customer evaluate Subconscious as a customer-hosted software vendor and software supply-chain vendor, rather than as a hosted production subservice provider operating the customer's inference environment.

## Scope note

For customer-hosted production deployments, Subconscious is a vendor organization, not a subservice organization. The customer does not rely on Subconscious operational controls to meet production service commitments because the runtime plane runs under the customer's deployment, access, monitoring, networking, and change-management controls.
...
- **Customer runtime plane**: The production system deployed into the customer's cloud. It handles inference traffic, routing, load balancing, users, API keys, and runtime observability.
- **Software distribution plane**: The software delivery and update workflow used to distribute new versions, patches, and release metadata to customer-controlled environments.

The customer runtime plane serves the customer's engineering users. It runs inside the customer's environment and is configured according to the customer's cloud, Kubernetes, networking, IAM, and security requirements.

The software distribution plane is used for delivering software. It is not required to receive customer code, prompts, completions, application logs, or production inference data.

## SOC 2 review position

Subconscious is designed to support the following review position for customer-hosted deployments:

> The product runs in the customer's cloud account. Subconscious does not host the production system, does not have persistent access, does not receive customer data, and does not control runtime availability. Updates are optional, customer-approved, signed where available, and installable through the customer's own change-management process.

This position depends on four operating principles:

- **No customer data processing by default**: Production inference traffic stays in the customer environment.
- **No required Subconscious cloud dependency for production operation**: The runtime plane should continue to operate under the customer's operational control.
...
For production customer-hosted deployments, the runtime plane is deployed in the customer's cloud and governed by the customer-controlled model described in this document.
...
- Telemetry statement.
- Deployment guide.
- FAQ for SOC 2 and vendor risk review.
...
Subconscious is designed so that the customer-hosted production deployment does not require the customer to rely on Subconscious operational controls for production availability, data processing, or runtime security.

For customer-hosted deployments, the key review point is that the runtime plane runs in the customer's cloud account, under the customer's access, monitoring, networking, and change-management controls. Subconscious can provide security assurance materials to support vendor risk review and SOC 2 audit discussions.

### Will deploying Subconscious compromise our SOC 2 audit?

The deployment is designed not to compromise the customer's SOC 2 audit posture.

The customer retains control over production deployment, runtime operation, monitoring, scanning, access, data, and version cadence. Subconscious does not require persistent production access and does not need customer data to operate the customer-hosted runtime plane.

Final audit treatment is determined by the customer and its auditor.

### Does Subconscious process or store our customer data?

For customer-hosted production deployments, Subconscious is not intended to process or store customer data. Inference requests, prompts, completions, logs, keys, and runtime data remain in the customer's environment unless the customer explicitly chooses to share specific data for support or troubleshooting.

### Does customer code leave our cloud?

No customer code should leave the customer's cloud in the customer-hosted production deployment unless the customer explicitly configures or shares it.
...
### Can Subconscious force an update into our environment?

No. Production updates should be customer-approved and deployed according to the customer's policy.
...
Customer-hosted deployments should not send customer data telemetry by default. Any telemetry should be explicit, documented, configurable, and opt-in.

### What happens during an incident?

The customer owns incident response for the runtime plane in its environment. Subconscious supports investigation and remediation when requested by the customer. Any access or data sharing during an incident should follow the customer's approval and logging process.
...
Customers should request the architecture memo, data-flow diagram, shared responsibility matrix, release integrity materials, vulnerability scan results, SBOMs, update process, support access policy, and telemetry statement.
