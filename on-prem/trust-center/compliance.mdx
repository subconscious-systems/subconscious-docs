...
The customer-hosted Subconscious Inference System is designed for enterprises that want high-performance coding-agent inference while retaining control over cloud environment, data, networking, ORANGE Line operations, and software deployment cadence.
...
- The customer controls versioning, deployment cadence, networking, access, monitoring, and ORANGE Line operations.
...
The ORANGE Line has three core components:
...
> The product runs in the customer's cloud account. Subconscious does not host the production system, does not have persistent access, does not receive customer data, and does not control production availability. Updates are optional, customer-approved, signed where available, and installable through the customer's own change-management process.
...
- **No required Subconscious cloud dependency for production operation**: The API Gateway and Inference Runtime should continue to operate under the customer's operational control.
...
Subconscious is designed so that the customer-hosted production deployment does not require the customer to rely on Subconscious operational controls for production availability, data processing, or API Gateway and Inference Runtime security.
...
The customer retains control over production deployment, API Gateway and Inference Runtime operation, monitoring, scanning, access, data, and version cadence. Subconscious does not require persistent production access and does not need customer data to operate The ORANGE Line.
