...
Your tool endpoint receives POST requests with a flat JSON body keyed by parameter name and returns JSON results.
...
Attach lifecycle-managed hosted resources, such as a sandbox or browser, by placing resource blocks in the same `tools` array as platform, function, and MCP tools.
...
tools: [
  ...
  tools.platform("web_search"),
  tools.platform("page_reader"),
